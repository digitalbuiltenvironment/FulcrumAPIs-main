...
def create_form(extension_data, issue_definition_id, project_id):
    load_dotenv(override=True)

    # Base url currently points to PROD, can be changed
    base_url = "https://optimus.fulcrumhq.build"
    endpoint = "/api/services/app/Issues/CreateOrEdit"
    ...
    headers = {
        ...
        'Lt-Ou': project_id, #change according to project
    }

    payload = {
        'issueDefinitionId': issue_definition_id, # change according to project
        'extensionData': json.dumps(extension_data),  # JSON string for nested data
        'organizationUnitId': project_id, # change according to project #same at Lt-Ou
        'sequenceType': 0,
        }    
